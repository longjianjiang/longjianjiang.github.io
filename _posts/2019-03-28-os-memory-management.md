...
基于这一思想，就产生了虚拟内存，同时这也是**覆盖技术**的替换方案。虚拟内存中，每个进程拥有独立的逻辑地址空间，此时内存按照分配单位的不同，分为页式存储和段式存储，两者被称为离散分配。
...
可以看到由两部分组成，前部分是页号，后部分是偏移量也即是页内地址。上图地址长度32为，0-11位是页内地址，每页大小为2KB；12-31位是页号，地址空间最多包含1M页。
