...
bp寄存器的存在在于sp寄存器是需要固定指向栈顶的，所以需要去在栈中找压入的某个数据，需要一个额外的标记位来进行定位，而bp寄存器就是扮演这样标记位的角色。
函数的每次调用bp寄存器会存储当前函数调用某个时刻sp的地址，此时取该地址所存储的值，就是上一个函数调用的sp地址，这样一层一层，就是我们看到的函数调用栈。
