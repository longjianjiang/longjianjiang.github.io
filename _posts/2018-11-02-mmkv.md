...
首先检查当前文件的可用size是否足够存储新添加的key和data，该步实现在方法`ensureMemorySize:`方法中， 主要为以下几个步骤：    
尝试加载本地文件，会调用到`loadFromFile`方法中，该方法首先根据文件大小和默认的内存页大小设置`m_size`，然后使用`mmap`建立文件内存映射。尝试读取当前文件的大小，设置`m_actualSize`。根据判断`m_actualSize`的大小和`m_actualSize`和`m_size`的大小关系，分别设置`m_output`和`m_dic`。这里忽略了crc校验和加密。    

比较newSize 和 文件剩余的size，如果够直接返回YES；否则将`m_size`扩大,重新建立文件内存映射，更新`m_output` 或者进行将文件中的内容去重。    

经过`ensureMemorySize:`后，如果内存够存放，当`m_actualSize`为0时直接将`m_dic`encode为data，将data写进文件中。当`m_actualSize`不为0时，则单独将key和data存进文件的末尾。    
