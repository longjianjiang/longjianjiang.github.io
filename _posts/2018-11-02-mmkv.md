...
write 浮点型没有使用varint，所以read 浮点型同样也不需要使用 read varint的方式，下面以32位来说明：  
...
- 1. size > 0 && size <= (m_size - m_position)
使用NSString的`initWithBytes:length:encoding`方法初始化string，position加上size，返回string。
