...
除此之外还提供了`autorelease` 的功能，当一个对象调用了该方法，会将该对象加入到内存池中，当包含该对象的内存池释放时，会首先将池中包含的对象释放。关于 `autorelease` 的更多内容可以看[这里](http://blog.sunnyxx.com/2014/10/15/behind-autorelease/)。
