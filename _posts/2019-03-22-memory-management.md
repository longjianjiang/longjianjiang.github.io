...
还有一种实现则是增加一个结构体用来存储分配的size，所以malloc方法返回的地址其实就是结构体的地址往前偏移了结构体size的一个新的地址，所以free的时候只需要将这个地址往后偏移结构体size就得到了结构体的地址，通过结构体地址自然也就知道了当初申请内存的大小。
...
笔者这里猜想函数参数使用时，使用`__autoreleasing`修饰的好处在于，方法内部创建的对象不需要调用者去调用release，交给pool去释放。到了ARC，其实使用`__strong`也是没有关系的，调用者创建的一个局部变量超过作用域，自动会调用一次release操作。
...
[https://segmentfault.com/a/1190000004665100](https://segmentfault.com/a/1190000004665100)
