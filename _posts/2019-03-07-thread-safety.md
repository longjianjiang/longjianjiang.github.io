...
死锁的检测：上锁前，查看该锁的持有者上的锁，从这些锁出发，再查看这些锁的持有者上的锁，如果发现这些锁中包含自己要上的锁，那说明检测到了死锁。有向图环路检测。
...
现在有一个问题，异步调用是如何实现的？之前我们提到过runloop，就是一个事件循环，其实但凡是GUI的程序都会有事件循环模型的，这个循环处于等待任务到来，执行任务，休眠。有了这个事件循环其实就可以异步调用了，当一个异步调用开始后，不会等待其结果而是先立即返回，因为此时可能需要继续往下执行后面的任务，当异步调用的耗时IO操作完成后，会发送一个类似操作完成的消息，事件循环接收到该消息后如果处于休眠则被唤醒去执行对应的回调，否则需要等待下一个事件循环。
...
> 笔者之前对同步异步的理解不准确，具体可以看[这里](http://www.longjianjiang.com/async/)。
